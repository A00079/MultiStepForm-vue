...
<section class="multi_step_form">  
  <form id="msform"> 
    ...
    <ul id="progressbar">
      ...
      <li :class="(showForm == 2 || showForm == 3)? 'active' : ''">Step 2</li> 
      <li :class="(showForm == 3)? 'active' : ''">Completed</li>
    </ul>
    <fieldset>
      <BaseFormOne v-show="showForm === 1" />
      <BaseFormTwo v-show="showForm === 2" />
      <BaseFormThree v-show="showForm === 3" />
    </fieldset>
  </form>  
</section> 
</template>
...
<script>
import BaseFormOne from '../BaseFromOne/Baseformone.vue';
import BaseFormTwo from '../BaseFormTwo/Baseformtwo.vue';
import BaseFormThree from '../BaseFormThree/Baseformthree.vue';
...
export default {
  ...
  mounted(){
    this.$root.$on('NEXT-FROM',(newval)=>{
      this.showForm = 0;
      this.showForm = newval
    });
    this.$root.$on('NEXT-FROM-SUBMIT',(newval)=>{
      this.showForm = 0;
      this.showForm = newval
    })
    this.$root.$on('BACK-TO-ONE',(newval)=>{
      this.showForm = 0;
      this.showForm = newval
    })
  },
